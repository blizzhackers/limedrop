  ...
  <Drawer direction="right" open={cartOpen} onOpenChange={setCartOpen}>
    <DrawerContent className="w-96 max-w-full h-full bg-gray-800 shadow-lg p-4 flex flex-col">
      <DrawerHeader className="flex items-center justify-between mb-4 p-0">
        <DrawerTitle className="text-2xl text-white font-bold">Drop List</DrawerTitle>
        <DrawerClose asChild>
      ...
      <form className="flex flex-col flex-1" onSubmit={e => { e.preventDefault(); handleDropCart(); }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {cart.length === 0 ? (
            ...
            cart.map((item, idx) => (
              <div key={item.itemid || idx} className="bg-gray-700 rounded p-2 mb-2 flex flex-row items-center">
                {item.image && (
                  <img
                    src={`data:image/jpeg;base64,${item.image}`}
                    alt={"item"}
                    className="ld-item mr-2"
                    style={{ maxWidth: 48, maxHeight: 48, imageRendering: 'crisp-edges' }}
                  />
                )}
                <div className="flex-1">
                  <div className="text-xs text-gray-400">{item.account} / {item.character} / {item.itemid}</div>
                </div>
                <button className="ml-2 text-red-400 hover:text-red-600" onClick={() => handleRemoveFromCart(item)} title="Remove" type="button">
